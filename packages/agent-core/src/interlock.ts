...
/**
 * Two independently NTP-synced clocks disagree by tens of milliseconds, so a
 * sample generated by the target arrives fractionally "in the future". Measured
 * against the live Cloud Run target this reached +117ms, which would otherwise
 * reset the evidence window on roughly every other sample and leave a healthy
 * hold unable to close.
 *
 * This is a calibration allowance for real hardware, not a policy knob: it is
 * deliberately not a contract field, because a proposer must never be able to
 * widen the tolerance that decides whether a target's clock is trustworthy. A
...
export function observe(
  ...
  const reason: ResetReason | undefined =
    ...
    (prior && observedAt <= prior.observedAt)
      ? "clock"
...
export function verify(
  ...
  const matches =
    ...
    observed.observedAt <= now + MAX_CLOCK_SKEW_MS &&
